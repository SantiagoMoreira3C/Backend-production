...
// Asegúrate de NO importar 'mongoose.Types.ObjectId' si usas UUIDs.
...
const getAllAssessments = async (req, res = response) => {
    try {
        ...
        // =================================================================
        // 🔑 PASO CLAVE: CONSTRUCCIÓN DEL ÚNICO $MATCH STAGE (UNIFICADO)
        // =================================================================

        // 1. Criterio de Seguridad/Filtro Principal para el campo 'psychologist'
       if (userRole === 'PSYCHOLOGIST') {
            // El ID del token (req.uid) debe convertirse si el _id de la colección users es ObjectId
            matchCriteria.psychologist = new ObjectId(psychologistId); 
            
        } else if (userRole === 'ADMIN') {
            const filterPsiId = filters.psychologistId;
            
            // Si el ADMIN envía un filtro de psicólogo en la URL:
            if (filterPsiId && filterPsiId.trim() !== '') {
                
                // 🚨 MODIFICACIÓN CRUCIAL: Usamos ObjectId.isValid() para asegurar el formato de 24 caracteres.
                if (ObjectId.isValid(filterPsiId)) {
                    // Si es un ID válido, lo convertimos a un objeto de MongoDB para el match.
                    matchCriteria.psychologist = new ObjectId(filterPsiId); 
                    console.log(`[ADMIN FILTRO MONGO] Aplicando filtro de ObjectId para el psicólogo: ${filterPsiId}`);
                } else {
                    // Esto solo debería pasar si se envía una cadena corrupta.
                    console.log(`[ADMIN ERROR MONGO] Recibido ID no válido para el filtro: ${filterPsiId}. IGNORANDO FILTRO.`);
                }
            }
        }

        // 2. Criterios de la Query (Fechas) - Se mantienen igual
        ...
        // 3. Aplicar el ÚNICO $match al inicio del pipeline
        if (Object.keys(matchCriteria).length > 0) {
            pipeline.push({ $match: matchCriteria });
        }
        
        // =================================================================
        // RESTO DE LA PIPELINE: $lookup y $project
        // =================================================================
        
        // La pipeline de lookup funciona con strings si los campos referenciados son strings.
        pipeline.push(
        ...
        pipeline.push(
            {
                $project: {
                    ...
                    // Devolvemos el ID del psicólogo para que el frontend pueda manejar el estado
                    'psychologistInfo._id': '$psychologist', // <-- Esto es el UUID
                }
            }
        );
        ...
        console.error('Error del servidor al obtener las evaluaciones:', error);
        res.status(500).json({ msg: 'Error del servidor al obtener las evaluaciones.' });
    }
};
// module.exports = { getAllAssessments }; // Exporta la función
